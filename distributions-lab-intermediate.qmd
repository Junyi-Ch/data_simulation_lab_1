...
# Complete this line:
acc_df <- tibble(acc = rbinom(___, size = ___, prob = ___))
...
acc_df_summary <- acc_df |>
  summarise(
    prop_correct = mean(___),  # Fill in the column name
    n = n()                    # This one is already done!
  )
...
# Complete this structure:
ggplot(___, aes(x = ___)) +  # Fill in dataframe and column name
  geom_histogram(binwidth = ___, alpha = ___) +  # Choose appropriate values
...
# Complete this structure:
ggplot(acc_df, aes(x = 'Overall', y = ___)) +  # Fill in the column name
...
# Complete this structure:
acc2 <- tibble(
  condition = rep(c("easy", "hard"), each = ___),  # Fill in number of trials per condition
  block = rep(rep(1:4, each = ___), times = 2),   # Fill in trials per block
  acc = c(
    rbinom(___, 1, 0.97),  # Fill in number of trials for easy condition
    rbinom(___, 1, 0.87)   # Fill in number of trials for hard condition
  )
)
...
acc_by_cb <- acc2 |>
  ...
  summarise(
    prop_correct = mean(___),  # Fill in column name
    trials = n(), 
    .groups = "drop"
  )
...
acc_wide <- acc_by_cb |>
  pivot_wider(
    names_from = ___,     # Fill in the column to become column names
    values_from = ___     # Fill in the column to become values
  )
...
# Complete this structure:
rt_df <- tibble(
  rt = r_shifted_lognorm(___, t0 = ___, meanlog = ___, sdlog = ___)  # Fill in the parameters
) |>
  mutate(log_rt = log(___))  # Fill in the column name
...
qplot(___, ___)  # Fill in the two column names

# Option 2: Using ggplot (more flexible)
ggplot(rt_df, aes(x = ___, y = ___)) +  # Fill in x and y column names
...
rt_df_summary <- rt_df |>
  summarise(
    ...
    median_rt = median(___), # Fill in column name
    max_rt = max(___),      # Fill in column name
    sd_rt = sd(___)         # Fill in column name
  )
...
rt_df_valid_invalid <- tibble(
  rt_valid = r_shifted_lognorm(___, t0 = ___, meanlog = ___, sdlog = ___),     # Fill in parameters for valid condition
  rt_invalid = r_shifted_lognorm(___, t0 = ___, meanlog = ___, sdlog = ___)   # Fill in parameters for invalid condition (slower)
)
...
rt_df_valid_invalid_longer <- rt_df_valid_invalid |>
  pivot_longer(
    cols = c(___, ___),        # Fill in the column names to pivot
    values_to = ___,           # Fill in the name for the values column
    names_to = ___             # Fill in the name for the names column
  )
...
valid_invalid_summary <- rt_df_valid_invalid_longer |>
  group_by(___) |>  # Fill in the grouping variable
  summarise(
    mean_rt = mean(___),  # Fill in column name
    sd_rt = sd(___),      # Fill in column name
    n = n()
...
# Complete this structure:
ggplot(___, aes(x = ___, fill = ___)) +  # Fill in dataframe and column names
  geom_density(alpha = ___) +            # Fill in alpha value (0-1)
